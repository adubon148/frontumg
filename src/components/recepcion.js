...
function Recepcion() {
    ...
    const [modalRecepcionados, setModalRecepcionados] = useState(false);
    ...
    const getPrestamos = async () => {
        await axios.get(baseurl + "all")
            .then(response => {
                setData(response.data.prestamos);
    ...
    const abrirCerrarModalRecepcionados = () => {
        setModalRecepcionados(!modalRecepcionados);
    }
    ...
    const bodyRecepcionados = (
        <div className={styles.modal}>
            <h3>Préstamos Recepcionados</h3>
            <TableContainer component={Paper}>
                <Table>
                    ...
                    <TableBody>
                        {data.filter(prestamo => prestamo.idestado === 2).map((prestamo) => (
            ...
            <Button onClick={abrirCerrarModalRecepcionados} color="primary">Cerrar</Button>
        </div>
    );
    ...
    return (
        <div className="App">
            ...
            <br /><br />
            <Button variant="contained" color="primary" onClick={abrirCerrarModalRecepcionados}>
                Ver los Recepcionados
            </Button>
            <TableContainer component={Paper}>
                <Table>
                    <TableHead>
                    ...
                    <TableBody>
                        {data.filter(prestamo => prestamo.idestado === 1).map((prestamo) => (
                            <TableRow key={prestamo.id}>
                                <TableCell>{prestamo.id}</TableCell>
                                <TableCell>{prestamo.fechaprestamo}</TableCell>
                                <TableCell>{libros.find(libro => libro.id === prestamo.idlibro)?.Titulo}</TableCell>
            ...
            <Modal
                open={modalRecepcionados}
                onClose={() => abrirCerrarModalRecepcionados()}>
                {bodyRecepcionados}
            </Modal>
        </div>
    );
